...
namespace Skylands
{
    ...
    public class BiomeControl : MonoBehaviour, Hoverable, Interactable
    {
        ...
        {
            if (!m_initialized || m_nview == null || !m_nview.IsValid() || m_nview.GetZDO() == null) return;
            ...
            if (ilhaPai != null && materiaisDoBioma != null && materiaisDoBioma.Count > 0)
            {
                ...
                if (biomeIndex < materiaisDoBioma.Count)
                {
                    ...
                    if (zsm != null)
                    {
                        bool isSnow = material != null && material.name.StartsWith("Snow", StringComparison.OrdinalIgnoreCase);
        ...
        {
            if (m_initialized) UpdateVisuals();
        }
        ...
        public string GetHoverText()
        {
            ...
            if (ArvoresObjects != null && ArvoresObjects.Count > 1)
            {
                ...
                texto += $"\n[<color=yellow><b>{TreeToggleKey}</b></color>] Trees ({(treesIndex == 0 ? "On" : "Off")})";
            }
            if (WaterWellObjects != null && WaterWellObjects.Count > 1)
            {
                int waterWellIndex = zdo.GetInt(ZDO_WATERWELL_INDEX, 0);
                texto += $"\n[<color=yellow><b>{WaterWellToggleKey}</b></color>] WaterWell ({(waterWellIndex == 0 ? "On" : "Off")})";
            }
            if (LampPostsObjects != null && LampPostsObjects.Count > 1)
            {
                int lampPostsIndex = zdo.GetInt(ZDO_LAMPPOSTS_INDEX, 0);
                texto += $"\n[<color=yellow><b>{LampPostsToggleKey}</b></color>] LampPosts ({(lampPostsIndex == 0 ? "On" : "Off")})";
            }
            if (materiaisDoBioma != null && materiaisDoBioma.Count > 0)
            {
                ...
                texto += $"\n[<color=yellow><b>{TextureToggleKey}</b></color>] Texture ({currentName})";
            }
